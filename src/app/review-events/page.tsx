...
import { getPendingEventDoubts, approveEventDoubt, rejectEventDoubt } from "../actions";
import { Loader2, CheckCircle, XCircle, AlertCircle, Calendar, ArrowRight, Search, ChevronRight, Zap } from "lucide-react";

function EventReviewContent() {
  ...
  const fetchDoubts = async () => {
    ...
    if (res.success && res.doubts) {
      ...
      if (!selectedId && res.doubts.length > 0) {
        ...
        router.replace(`/review-events?id=${firstId}`);
  ...
  const handleSelectDoubt = (id: string) => {
    setSelectedId(id);
    setReviewerNotes("");
    router.replace(`/review-events?id=${id}`);
  };

  const handleApprove = async (id: string) => {
    setProcessingId(id);
    const res = await approveEventDoubt(id, reviewerNotes);
    if (res.success) {
      showToast("Successfully linked event to the Golden Profile.", "approve");
      setReviewerNotes("");
      ...
      if (selectedId === id) {
        if (nextDoubts.length > 0) {
          ...
          router.replace(`/review-events?id=${nextId}`);
        } else {
          setSelectedId(null);
          router.replace('/review-events');
        }
      }
    } else {
  ...
  const handleReject = async (id: string) => {
    setProcessingId(id);
    const res = await rejectEventDoubt(id, reviewerNotes);
    if (res.success) {
      showToast("Discarded orphaned event.", "reject");
      setReviewerNotes("");
      const nextDoubts = doubts.filter(d => d.id !== id);
      setDoubts(nextDoubts);
  ...
  const filteredDoubts = doubts.filter(d => 
    d.activityEvent?.businessName?.toLowerCase().includes(searchQuery.toLowerCase()) || 
      ...
      <div className="container">
        <div className="card" style={{ textAlign: "center", padding: "4rem" }}>
          ...
          <p style={{ color: "var(--text-secondary)" }}>
            There are no pending events requiring human review at this time.
          </p>
  ...
  return (
    <div style={{ maxWidth: "1400px", margin: "0 auto", padding: "1.5rem 2rem", height: "calc(100vh - 65px)", display: "flex", flexDirection: "column" }}>
      <div style={{ marginBottom: "1rem", flexShrink: 0 }}>
        <h1 className="title" style={{ fontSize: "1.75rem", marginBottom: "0.25rem", display: "flex", alignItems: "center", gap: "0.5rem" }}><Zap color="var(--primary-color)" /> Event Streams Review</h1>
        <p className="subtitle" style={{ marginBottom: 0 }}>
          {doubts.length} unlinked events require human verification
        </p>
      </div>
                  ...
                  >
                    <div style={{ overflow: "hidden", paddingRight: "0.5rem" }}>
                      <div style={{ fontWeight: isSelected ? "600" : "500", fontSize: "0.875rem", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", marginBottom: "0.25rem", color: isSelected ? "var(--primary-color)" : "var(--text-primary)" }}>
                        {doubt.activityEvent?.businessName || doubt.activityEvent?.departmentId || "Unknown Event"}
                      </div>
                      <div style={{ fontSize: "0.75rem", color: "var(--text-secondary)", display: "flex", gap: "0.5rem" }}>
                        ...
                        <span>&middot;</span>
                        <span>{doubt.activityEvent?.eventType}</span>
                      </div>
                    </div>
            ...
            <div style={{ opacity: processingId === selectedDoubt.id ? 0.5 : 1, transition: "opacity 0.2s" }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px solid var(--border-color)", paddingBottom: "1.5rem", marginBottom: "2rem" }}>
                <div>
                  <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", marginBottom: "0.25rem" }}>
                    ...
                    <h2 style={{ fontWeight: "600", fontSize: "1.25rem", margin: 0 }}>Unlinked Activity Event</h2>
                  </div>
                  <p style={{ color: "var(--text-secondary)", fontSize: "0.875rem", margin: 0 }}>
                    AI detected a {(selectedDoubt.confidence * 100).toFixed(1)}% match to a Golden Profile. Please verify.
                  </p>
                </div>
              </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 40px 1fr", gap: "1.5rem", alignItems: "start", marginBottom: "2rem" }}>
                ...
                <div style={{ backgroundColor: "rgba(255,255,255,0.02)", padding: "1.5rem", borderRadius: "12px", border: "1px dashed var(--border-color)" }}>
                  <div style={{ fontSize: "0.75rem", textTransform: "uppercase", letterSpacing: "0.05em", color: "var(--text-secondary)", marginBottom: "1rem", fontWeight: "600", display: "flex", justifyContent: "space-between" }}>
                    ...
                    <span style={{ backgroundColor: "rgba(255,255,255,0.1)", padding: "0.15rem 0.5rem", borderRadius: "4px", color: "#fff" }}>{selectedDoubt.activityEvent.departmentName}</span>
                  </div>
                  <div style={{ fontWeight: "600", fontSize: "1.25rem", display: "flex", alignItems: "flex-start", gap: "0.75rem", marginBottom: "1rem" }}>
                    <Calendar size={20} color="var(--primary-color)" style={{ marginTop: "0.25rem", flexShrink: 0 }} /> 
                    <span>{selectedDoubt.activityEvent.businessName || "Unknown Business Name"}</span>
                  </div>
                  <div style={{ color: "var(--text-secondary)", fontSize: "0.875rem", marginBottom: "1.5rem", lineHeight: "1.6" }}>
                    <strong>Event Type:</strong> {selectedDoubt.activityEvent.eventType} <br/>
                    <strong>Date:</strong> {new Date(selectedDoubt.activityEvent.eventDate).toLocaleDateString()} <br/>
                    <strong>Address:</strong> {selectedDoubt.activityEvent.address || "N/A"}
                  </div>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem", fontSize: "0.875rem" }}>
                    <div style={{ backgroundColor: "var(--bg-color)", padding: "0.75rem", borderRadius: "6px", border: "1px solid var(--border-color)" }}>
                      ...
                      <strong style={{ fontFamily: "monospace", fontSize: "1rem" }}>{selectedDoubt.activityEvent.pan || "-"}</strong>
                    </div>
                    <div style={{ backgroundColor: "var(--bg-color)", padding: "0.75rem", borderRadius: "6px", border: "1px solid var(--border-color)" }}>
                      <span style={{ color: "var(--text-secondary)", display: "block", fontSize: "0.75rem", marginBottom: "0.25rem" }}>GSTIN</span>
                      <strong style={{ fontFamily: "monospace", fontSize: "1rem" }}>{selectedDoubt.activityEvent.gstin || "-"}</strong>
                    </div>
                  </div>
                </div>
                ...
                {selectedDoubt.unifiedRecord ? (
                  <div style={{ backgroundColor: "rgba(59, 130, 246, 0.03)", padding: "1.5rem", borderRadius: "12px", border: "1px solid rgba(59, 130, 246, 0.3)" }}>
                    <div style={{ fontSize: "0.75rem", textTransform: "uppercase", letterSpacing: "0.05em", color: "var(--primary-color)", marginBottom: "1rem", fontWeight: "600", display: "flex", justifyContent: "space-between" }}>
                      <span>Proposed Golden Profile</span>
                      <span style={{ backgroundColor: "rgba(59, 130, 246, 0.1)", padding: "0.15rem 0.5rem", borderRadius: "4px" }}>UBID</span>
                    </div>
                    <div style={{ fontWeight: "600", fontSize: "1.25rem", display: "flex", alignItems: "flex-start", gap: "0.75rem", marginBottom: "1rem" }}>
                      <Zap size={20} color="var(--primary-color)" style={{ marginTop: "0.25rem", flexShrink: 0 }} /> 
                  ...
                  <div style={{ backgroundColor: "rgba(255,255,255,0.02)", padding: "1.5rem", borderRadius: "12px", border: "1px dashed var(--border-color)", display: "flex", alignItems: "center", justifyContent: "center", color: "var(--text-secondary)", textAlign: "center", height: "100%" }}>
                    No Golden Profile match found. <br/> This event is currently orphaned.
                  </div>
                )}
              </div>
              ...
              <div style={{ borderTop: "1px solid var(--border-color)", paddingTop: "1.5rem", display: "flex", gap: "1.5rem", alignItems: "flex-end" }}>
                <div style={{ flex: 1 }}>
                  ...
                  <input 
                    type="text" 
                    placeholder="E.g., Verified the premises online..." 
                    value={reviewerNotes}
                ...
                <div style={{ display: "flex", gap: "0.75rem" }}>
                  ...
                  >
                    <XCircle size={18} /> Discard Event
                  </button>
                  <button 
                    className="btn" 
                    onClick={() => handleApprove(selectedDoubt.id)}
                    disabled={!!processingId || selectedDoubt.ubid === "ORPHAN"}
                    style={{ backgroundColor: "var(--success-color)", color: "#fff", padding: "0.75rem 1.5rem", display: "flex", alignItems: "center", gap: "0.5rem", opacity: selectedDoubt.ubid === "ORPHAN" ? 0.5 : 1 }}
                  >
...
export default function EventReviewPortal() {
  return (
    <Suspense fallback={
      <div className="container" style={{ display: "flex", justifyContent: "center", paddingTop: "5rem" }}>
        <Loader2 size={32} className="spin" style={{ color: "var(--primary-color)" }} />
